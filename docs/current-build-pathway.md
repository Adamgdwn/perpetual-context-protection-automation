...
- 2026-07-09T18:05:00-06:00: Live-testing prep surfaced clipped output in a managed Codex terminal (operator saw only scrollback fragments, no input box). Root cause was a fixed pty size: the companion started sessions without dimensions, so the bridge spawned every pty at a 100x30 fallback and never resized it, while a full-screen TUI agent drew its input box/status lines below a shorter terminal panel. Fixed by forwarding the real terminal dimensions to the bridge on open and on every resize (new `POST /sessions/:id/resize` route → `ManagedPtySession.resize`), so the live pty tracks what the operator sees. Added a bridge resize test. This activates per VS Code window on reload; existing live sessions keep their spawn-time size until restarted. Immediate no-reload workaround for a clipped session: enlarge the terminal above the 100x30 grid (maximize the panel or move the terminal into the editor area).
...
| 2026-07-09T18:05:00-06:00 | `npm run lint` | pass | ESLint clean after terminal resize-forwarding change. |
| 2026-07-09T18:05:00-06:00 | `npm test` | pass | 30 unit tests, including new `POST /sessions/:id/resize` coverage (valid/invalid/unknown). |
| 2026-07-09T18:05:00-06:00 | `npm run build` | pass | TypeScript compile and desktop renderer production build passed. |
| 2026-07-09T18:05:00-06:00 | `env -u ELECTRON_RUN_AS_NODE npm run desktop:smoke` | pass | Desktop shell + Operator Guide rendered; exit 0. |
